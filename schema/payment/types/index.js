...
const PaymentTypes = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    paymentDate: {
      type: GraphQLString,
      resolve: (src) =>
        src.paymentDate ? new Date(src.paymentDate).toISOString() : null,
    },
    createdAt: {
      type: GraphQLString,
      resolve: (src) =>
        src.createdAt ? new Date(src.createdAt).toISOString() : null,
    },
    updatedAt: {
      type: GraphQLString,
      resolve: (src) =>
        src.updatedAt ? new Date(src.updatedAt).toISOString() : null,
    },
    deletedAt: {
      type: GraphQLString,
      resolve: (src) =>
        src.deletedAt ? new Date(src.deletedAt).toISOString() : null,
    },
    ...
    expiredDate: {
      type: GraphQLString,
      resolve: (src) =>
        src.expiredDate ? new Date(src.expiredDate).toISOString() : null,
    },
  }),
});
